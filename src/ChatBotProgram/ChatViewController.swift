...
class ChatViewController: UIViewController, UITextViewDelegate, UITableViewDataSource, UITableViewDelegate, LoanButtonPressedDelegate  {

    let randomReply = ["Done...!", "You can count on it", "Ye boy", "Cheeky", "Added that for you", "I can't let that slip though"]
    ...
    override func viewDidLoad() {
        ...
        tableView.register(UINib(nibName: "LoanViewCell", bundle: nil), forCellReuseIdentifier: "LoanViewCell")
    ...
    func addPressed() {
        memories.append(Loan(text: textView!.text))
        
        messages.removeLast()
        
        tableView.re.deleteRows(at: [IndexPath(row: messages.count , section: 0)], with: .middle)
        self.toolbar.setNeedsDisplay()
        
        sendMessage(Message(text: "🎉🎉🎉", date: Date(), type: .botText))
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .botText:
            ...
            return cell
        case .loanMessage:
            let cell = tableView.dequeueReusableCell(withIdentifier: "LoanViewCell", for: indexPath) as! LoanViewCell
            cell.configure(with: message)
            cell.delegate = self
            return cell
        }
    ...
    @objc func send() {
        ...
        if self.textView!.text != "" {
            ...
            if self.textView!.text.contains("owe") {
                //            let message = Message(text: self.textView!.text!, date: Date(), type: .botForecast)
                //
                //            self.sendMessage(message)
//                var pars = OwerParser()
// 
//                var arr = pars.getAllIous()
//                var string = ""
//                for i in arr {
//                    string += i
//                }
//                print("array is: \(arr)")
//                print(string)
                let string = "Would you like me to remind you about your friend's borrowings?"
                let message2 = Message(text: string, date: Date(), type: .loanMessage)
                
                self.sendMessage(message2)

            }
            
            if self.textView!.text.contains("meet") {
                //            let message = Message(text: self.textView!.text!, date: Date(), type: .botForecast)
                //
                //            self.sendMessage(message)
                
                let message2 = Message(text: "Set up meetup time and date?", date: Date(), type: .loanMessage)
            ...
            let message = Message(text: "You're just going to be like that, aren't you, user? How rude!", date: Date(), type: .botText)
    ...
    func sendWelcomeMessage() {
        let firstTime = true
        if firstTime {
            let text = "Welcome back! I'm here to assist your group!"
